    ...
    <div className="p-6 min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white">
      ...
      <div className="bg-indigo-900/40 border border-indigo-700/40 p-6 rounded-2xl shadow mb-10">
        ...
        <ul className="space-y-2">
          {expense > income && <li className="text-red-400">⚠️ You are overspending! Reduce expenses immediately.</li>}
          {balance < 1000 && <li className="text-yellow-400">⚠️ Balance is very low — limit spending this week.</li>}
          {biggestSpend > 0 && (
            <li className="text-blue-300">
              💡 Most money goes to <b>{biggestCategory}</b> (₹{biggestSpend}). Cutting 20% saves ₹{(0.2 * biggestSpend).toFixed(0)}.
            </li>
          )}
          <li className="text-green-300">📊 Saving Rate: {savingRate}% — aim for at least 20%.</li>
          {daysLeft > 0 && <li className="text-cyan-300">⏳ Balance will last ~{daysLeft} days at current pace.</li>}
        </ul>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-slate-900/60 p-6 rounded-2xl shadow">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={60} outerRadius={120} label={(entry) => `${entry.name} (${entry.percent}%)`}>
